...
scanners.each_with_index do |scanner, i|
  parent, absolute_position, current_rotation = scanner_positions[i]

  while parent
    parent, position, rotation = scanner_positions[parent]

    absolute_position = rotate(absolute_position, rotation)
    current_rotation = rotate(current_rotation, rotation)
    3.times.each { |j| absolute_position[j] += position[j] }
  end

  scanner = scanner.map { |beacon| rotate(beacon, current_rotation) }
  scanner = add_offset_to_beacons(scanner, absolute_position)
 
  scanner.each { |beacon| all_beacons[beacon] = true }
end
