...
const remove = (supporting, supportedBy, name) => {
  let total = 0;
  const cubesToRemove = [name];

  supporting = copy(supporting);
  supportedBy = copy(supportedBy);

  while (cubesToRemove.length > 0) {
    const cube = cubesToRemove.shift();

    Object.keys(supporting[cube] || {}).forEach((c) => {
      delete supportedBy[c][cube];

      if (Object.keys(supportedBy[c]).length === 0) {
        cubesToRemove.push(c);
      }
    });

    delete supporting[cube];
    total++;
  }
...
const result = cubes.reduce(
  (sum, c) => sum + remove(supporting, supportedBy, c.name),
  0
);
console.log(result);
