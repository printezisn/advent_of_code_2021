...
let startPositions = [], endPosition = [0, 0], i, j;

for (i  = 0; i < grid.length; i++) {
  for (j = 0; j < grid[i].length; j++) {
    if (grid[i][j] === 'S' || grid[i][j] === 'a') startPositions.push([i, j]);
    else if (grid[i][j] === 'E') endPosition = [i, j];
  }
}

const minSteps = startPositions.reduce((min, startPosition) => {
  const result = calculateMinSteps(grid, startPosition, endPosition);
  if (result == null) return min;
  if (min == null) return result;
