...
const calculateMaxGeodes = (
  ...
  let maxGeode = null, bestOutcome = resources.geode, totalGeodeRobots = robots.geode;

  for (let i = 0; i < remainingMinutes; i++) {
    bestOutcome += totalGeodeRobots;
    totalGeodeRobots++;
  }

  if (bestOutcome <= maxScore.value) return 0;
  ...
  Object.keys(blueprint).forEach(robotType => {
    ...
    maxGeode = Math.max(
      maxGeode || 0,
      calculateMaxGeodes(blueprint, maxResourcesPerBlueprint, remainingMinutes - requiredMinutes, newResources, newRobots, maxScore)
    );
  });
  ...
  maxScore.value = Math.max(maxScore.value, maxGeode);
...
blueprints.slice(0, 3).forEach((blueprint, i) => {
  const quality = calculateMaxGeodes(blueprint, maxResourcesPerBlueprint[i], 32);
